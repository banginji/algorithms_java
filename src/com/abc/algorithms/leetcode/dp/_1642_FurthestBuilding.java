...
public class _1642_FurthestBuilding {
    private static int furthestBuilding(int[] heights, int bricks, int ladders) {
        if (bricks == 0 && ladders == 0) {
            int result = 0, itrIdx = 1;
            while (heights[itrIdx] < heights[itrIdx++ - 1]) result++;
            return result;
        }
        return furthestBuilding(1, heights, bricks, ladders);
    }

    private static int furthestBuilding(int heightIdx, int[] heights, int bricks, int ladders) {
        if (heightIdx > heights.length - 1) return 0;
        if (heights[heightIdx - 1] >= heights[heightIdx]) return 1 + furthestBuilding(heightIdx + 1, heights, bricks, ladders);
        int b = bricks >= (heights[heightIdx] - heights[heightIdx - 1]) ? 1 + furthestBuilding(heightIdx + 1, heights, bricks - (heights[heightIdx] - heights[heightIdx - 1]), ladders) : 0;
        int l = ladders > 0 ? 1 + furthestBuilding(heightIdx + 1, heights, bricks, ladders - 1) : 0;
        return Math.max(b, l);
    }

    private static record DpKey(int heightIdx, int bricks, int ladders) {}

    // Does not work for very huge values like the large input in the test cases
    private static int furthestBuildingDp(int[] heights, int bricks, int ladders) {
        if (bricks == 0 && ladders == 0) {
            int result = 0, itrIdx = 1;
            while (heights[itrIdx] < heights[itrIdx++ - 1]) result++;
            return result;
        ...
        return furthestBuildingDp(1, heights, bricks, ladders, dp);
    }

    private static int furthestBuildingDp(int heightIdx, int[] heights, int bricks, int ladders, Map<DpKey, Integer> dp) {
        if (heightIdx > heights.length - 1) return 0;
        DpKey currentDpKey = new DpKey(heightIdx, bricks, ladders);
        if (dp.containsKey(currentDpKey)) return dp.get(currentDpKey);
        if (heights[heightIdx - 1] >= heights[heightIdx]) {
            dp.put(currentDpKey, 1 + furthestBuildingDp(heightIdx + 1, heights, bricks, ladders, dp));
            return dp.get(currentDpKey);
        }
        int b = bricks >= (heights[heightIdx] - heights[heightIdx - 1]) ? 1 + furthestBuildingDp(heightIdx + 1, heights, bricks - (heights[heightIdx] - heights[heightIdx - 1]), ladders, dp) : 0;
        int l = ladders > 0 ? 1 + furthestBuildingDp(heightIdx + 1, heights, bricks, ladders - 1, dp) : 0;
        dp.put(currentDpKey, Math.max(b, l));
        return dp.get(currentDpKey);
    }

    private static int furthestBuildingHeap(int[] heights, int bricks, int ladders) {
        PriorityQueue<Integer> pq = new PriorityQueue<>();
        for (int heightIdx = 1; heightIdx < heights.length; heightIdx++) {
            int heightDiff = heights[heightIdx] - heights[heightIdx - 1];
            if (heightDiff > 0) pq.offer(heightDiff);
            if (pq.size() > ladders && pq.size() > 0) bricks -= pq.poll();
            if (bricks < 0) return heightIdx - 1;
        }
